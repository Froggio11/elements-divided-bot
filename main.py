...
async def _gs(k):
    r = await get_redis(); v = await r.get(f"edl:{k}"); return json.loads(v) if v else None
async def _ss(k, v):
    r = await get_redis(); await r.set(f"edl:{k}", json.dumps(v))
async def _ds(k):
    r = await get_redis(); await r.delete(f"edl:{k}")
